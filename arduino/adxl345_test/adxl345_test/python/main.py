...
# ---------------- Config ----------------
AI_PC_IP    = "10.91.62.10"   # <<< CHANGE to your AI PC's IP (ipconfig on the PC)
AI_PC_PORT  = 5000
...
def loop():
    ...
    if len(window) == window.maxlen and (t0 - _last_pred_t) >= PREDICT_EVERY:
        ...
        # Mirror the status line on the Serial Monitor via the MCU
        try:
            Bridge.call("show_status", status)
        except Exception:
            pass
